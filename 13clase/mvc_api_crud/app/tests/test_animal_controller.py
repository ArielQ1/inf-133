...
def test_get_animals_user(test_client, auth_headers1):
    response = test_client.get("/api/animals", headers=auth_headers1)
    assert response.status_code == 200
    assert response.json == []

def test_get_animals_id_user(test_client, auth_headers1):
    response = test_client.get("/api/animals/1", headers=auth_headers1)
    assert response.status_code == 404

def test_create_animal_user(test_client, auth_headers1):
    data = {"name": "Lion", "species": "Panthera leo", "age": 5}
    response = test_client.post("/api/animals", json=data, headers=auth_headers1)
    assert response.status_code == 201
    assert response.json["name"] == "Lion"

def test_update_animal(test_client, auth_headers1):
    # Primero crea un animal
    data = {"name": "Elephant", "species": "Loxodonta", "age": 10}
    response = test_client.post("/api/animals", json=data, headers=auth_headers1)
    assert response.status_code == 201
    ...
    update_data = {"name": "Elephant", "species": "Loxodonta africana", "age": 12}
    response = test_client.put(
        f"/api/animals/{animal_id}", json=update_data, headers=auth_headers1
    )
    assert response.status_code == 200
    assert response.json["species"] == "Loxodonta africana"
    assert response.json["age"] == 12

def test_delete_animal(test_client, auth_headers1):
    # Primero crea un animal
    data = {"name": "Giraffe", "species": "Giraffa camelopardalis", "age": 7}
    response = test_client.post("/api/animals", json=data, headers=auth_headers1)
    assert response.status_code == 201
    ...
    # Ahora elimina el animal
    response = test_client.delete(f"/api/animals/{animal_id}", headers=auth_headers1)
    assert response.status_code == 204

    # Verifica que el animal ha sido eliminado
    response = test_client.get(f"/api/animals/{animal_id}", headers=auth_headers1)
    assert response.status_code == 404